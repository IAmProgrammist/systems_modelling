...
double getC(double angle) {
    return std::sqrt(r*r + r*r - 2*r*r*std::cos(degreeToRadians(angle)));
}

double getDeltaL(double angle) {
    return std::sqrt(getC(angle) * getC(angle) + l0*l0 - 2*l0*getC(angle)*std::cos(degreeToRadians(180 - angle / 2))) - l0;
}

double getAngleAcc(double angle) {
    return -(
    // M1
    k1 * getDeltaL(angle) * r +
